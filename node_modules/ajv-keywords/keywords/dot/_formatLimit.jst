...
{{? $isDataFormat }}
  {{
    var $schemaValueFormat = it.creativei.util.getData($schemaFormat.$data, $dataLvl, it.dataPathArr)
      , $format = 'format' + $lvl
      , $compare = 'compare' + $lvl;
  }}
...
{{??}}
  ...
  {{? !($format && $format.compare) }}
    {{# def.skipFormatLimit }}
  {{?}}
  {{ var $compare = 'formats' + it.creativei.util.getProperty($schemaFormat) + '.compare'; }}
{{?}}
...
{{? $isDataExcl }}
  {{
    var $schemaValueExcl = it.creativei.util.getData($schemaExcl.$data, $dataLvl, it.dataPathArr)
