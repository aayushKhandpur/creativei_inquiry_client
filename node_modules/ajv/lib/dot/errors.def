  ...
  {{? it.createErrors !== false }}
    {
      ...
      , schemaPath: {{=it.creativei.util.toQuotedString($errSchemaPath)}}
...
{{## def.concatSchemaEQ:{{?$isData}}' + {{=$schemaValue}} + '{{??}}{{=it.creativei.util.escapeQuotes($schema)}}{{?}}#}}

{{## def._errorMessages = {
  'false schema':  "'boolean schema is false'",
  $ref:            "'can\\\'t resolve reference {{=it.creativei.util.escapeQuotes($schema)}}'",
  additionalItems: "'should NOT have more than {{=$schema.length}} items'",
  additionalProperties: "'should NOT have additional properties'",
  anyOf:           "'should match some schema in anyOf'",
  const:           "'should be equal to constant'",
  contains:        "'should contain a valid item'",
  dependencies:    "'should have {{? $deps.length == 1 }}property {{= it.creativei.util.escapeQuotes($deps[0]) }}{{??}}properties {{= it.creativei.util.escapeQuotes($deps.join(\", \")) }}{{?}} when property {{= it.creativei.util.escapeQuotes($property) }} is present'",
  ...
  patternGroups:   "'should NOT have {{=$moreOrLess}} than {{=$limit}} properties matching pattern \"{{=it.creativei.util.escapeQuotes($pgProperty)}}\"'",
...
{{## def.schemaRefOrQS: {{?$isData}}validate.schema{{=$schemaPath}}{{??}}{{=it.creativei.util.toQuotedString($schema)}}{{?}} #}}

{{## def._errorSchemas = {
  'false schema':  "false",
  $ref:            "{{=it.creativei.util.toQuotedString($schema)}}",
...
{{## def.schemaValueQS: {{?$isData}}{{=$schemaValue}}{{??}}{{=it.creativei.util.toQuotedString($schema)}}{{?}} #}}

{{## def._errorParams = {
  'false schema':  "{}",
  $ref:            "{ ref: '{{=it.creativei.util.escapeQuotes($schema)}}' }",
  additionalItems: "{ limit: {{=$schema.length}} }",
  additionalProperties: "{ additionalProperty: '{{=$additionalProperty}}' }",
  anyOf:           "{}",
  const:           "{}",
  contains:        "{}",
  dependencies:    "{ property: '{{= it.creativei.util.escapeQuotes($property) }}', missingProperty: '{{=$missingProperty}}', depsCount: {{=$deps.length}}, deps: '{{= it.creativei.util.escapeQuotes($deps.length==1 ? $deps[0] : $deps.join(\", \")) }}' }",
  ...
  patternGroups:   "{ reason: '{{=$reason}}', limit: {{=$limit}}, pattern: '{{=it.creativei.util.escapeQuotes($pgProperty)}}' }",
