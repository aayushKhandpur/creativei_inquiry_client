...
{{## def.validateItems:startFrom:
  for (var {{=$idx}} = {{=startFrom}}; {{=$idx}} < {{=$data}}.length; {{=$idx}}++) {
    {{
      $it.errorPath = it.creativei.util.getPathExpr(it.errorPath, $idx, it.opts.jsonPointers, true);
    ...
    {{? {{# def.nonEmptySchema:$sch }} }}
      ...
      if ({{=$data}}.length > {{=$i}}) {
        {{
          ...
          $it.errorPath = it.creativei.util.getPathExpr(it.errorPath, $i, it.opts.jsonPointers, true);
