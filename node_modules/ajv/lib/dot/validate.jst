...
{{
  var $async = it.schema.$async === true
    , $refKeywords = it.creativei.util.schemaHasRulesExcept(it.schema, it.RULES.all, '$ref')
...
{{## def.checkType:
  ...
  if ({{= it.creativei.util[$method]($typeSchema, $data, true) }}) {
#}}
...
{{? $typeSchema }}
  {{? it.opts.coerceTypes }}
    {{ var $coerceToTypes = it.creativei.util.coerceToTypes(it.opts.coerceTypes, $typeSchema); }}
  {{?}}
...
{{??}}
  ...
  {{~ it.RULES:$rulesGroup }}
    {{? $shouldUseGroup($rulesGroup) }}
      {{? $rulesGroup.type }}
        if ({{= it.creativei.util.checkDataType($rulesGroup.type, $data) }}) {
      {{?}}
